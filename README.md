...
## Quick Start
`bash <( curl -L https://raw.githubusercontent.com/RasPlex/jenkins-slave-osx/master/install.sh )`
...
## Install
`bash <( curl -L https://raw.githubusercontent.com/RasPlex/jenkins-slave-osx/master/install.sh ) [options]`
...
* `--secret=SECRET` to specify the Jenkins secret for the slave. *Optional;* the installer prompts for this if not specified on the command line.
...
## Configuration
The file ``Library/Preferences/org.jenkins-ci.slave.jnlp.conf`` in ``/Users/jenkins`` (assuming an installation in the default location) can be used to configure this service with these options:
...
2. `sudo cp /path/to/exported-keys-and-certificates /Users/jenkins`
3. For each certificate and key:
   `sudo -i -u jenkins /Users/jenkins/Library/Developer/org.jenkins-ci.slave.jnlp/security.sh add-apple-certificate --certificate=/Users/jenkins/name-of-exported-cert`
...
1. `sudo launchctl unload /Users/jenkins/Library/LaunchAgents/org.jenkins-ci.slave.jnlp.plist`
2. `sudo -i -u jenkins`
3. `curl -O http://www.startssl.com/certs/ca.crt`
4. `/Users/jenkins/Library/Developer/org.jenkins-ci.slave.jnlp/security.sh add-java-certificate --authority --alias=root-ca --certificate=./ca.crt`
5. `curl -O http://www.startssl.com/certs/sub.class1.server.ca.crt`
6. `/Users/jenkins/Library/Developer/org.jenkins-ci.slave.jnlp/security.sh add-java-certificate --alias=ca-server --certificate=./sub.class1.server.ca.crt`
7. `rm ./*ca.crt`
8. `exit`
9. `sudo launchctl load /Users/jenkins/Library/LaunchAgents/org.jenkins-ci.slave.jnlp.plist`
